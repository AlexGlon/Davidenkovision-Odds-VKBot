...
def get_bet_category_to_bet_on(**kwargs) -> tuple[str, dict]:
    ...
    query = (
        "SELECT row_number() OVER (ORDER BY betting_categories.betting_category_id), betting_category_id, "
    ...
    query = (
        "SELECT user_id, total, balances.balance_id, contest_id "
        "FROM balances "
    ...
    if len(balances) == 0:
        ...
        query = (
            "INSERT INTO balances (user_id, total, balance_id) "
        ...
        for contest in contests_to_bet_on:
            query = (
                "INSERT INTO balances_contests (balance_id, contest_id) "
        ...
        conn.commit()
        logging.info(
            f"Created balance->contest entries for balance {balance_id} and contests {contests_to_bet_on}"
        )
...
def get_entry_to_bet_on(**kwargs) -> tuple[str, dict]:
    ...
    query = (
        "SELECT row_number() OVER (ORDER BY entries.entry_id), entries.entry_id, "
    ...
    extra_info["coefficients"] = tuple(
        [coefficient_calculation(entry[6]) for entry in entries]
    )
    extra_info["entries_to_bet_on"] = tuple([entry[1] for entry in entries])
    extra_info["entries_to_bet_on_listed_number"] = tuple(
        [entry[0] for entry in entries]
    )
    extra_info["selected_category_id"] = db_category_id
    extra_info["selected_contest_id"] = extra_info.get("contest_ids")[
        selected_category - 1
    ]
...
def validate_and_accept_incoming_bet(**kwargs) -> tuple[str, dict]:
    ...
    selected_entry_id, points_to_spend = kwargs.get(
        "invoking_message", "-999 -999"
    ).split()

    current_points = extra_info.get("current_points_on_balance", "-999")
    valid_entries = extra_info.get("entries_to_bet_on_listed_number", ())

    if (int(selected_entry_id) not in valid_entries) or (
        int(points_to_spend) > current_points or int(points_to_spend) <= 0
    ):
    ...
    conn.commit()
    logging.info(
        f"Bet has been placed by user {user_id}: "
...
def bet_processing(user_id, arg_list, tokens_available, vk):
    ...
    if current_bet["entry_id"] < 1 or current_bet["entry_id"] > 26:
        write_msg(
            user_id,
            "Неверный код страны! Попробуйте ввести запрос снова или введите 'выход' для выхода.",
            vk,
        )
        return False
    if current_bet["tokens"] < 1 or current_bet["tokens"] > tokens_available:
        write_msg(
            user_id,
            "Неверное количество фишек! Попробуйте ввести запрос снова или введите 'выход' для выхода.",
            vk,
        )
        return False
    ...
    write_msg(
        user_id,
        f"Ставка на заявку {current_bet['entry_id']} в количестве {current_bet['tokens']} фишек с коэффициентом {current_bet['coefficient']} принята! При желании, её можно снять командой 'удалить ставку'.",
        vk,
    )
    return True
...
def entry_point(user_id, longpoll, vk):
    ...
    if tokens_available == 0:
        write_msg(
            user_id,
            "Вы уже использовали все фишки! Вы никогда не были крохобором...",
            vk,
        )
        return
    else:
        write_msg(
            user_id,
            f"Давайте сделаем ставку! У вас есть {tokens_available} фишек.\nЧтобы сделать ставку, введите порядковый номер заявки и количество фишек в формате 'заявка фишки', к примеру, '1 99'.\nВведите 'заявки', если вы хотите увидеть список заявок.",
            vk,
        )
